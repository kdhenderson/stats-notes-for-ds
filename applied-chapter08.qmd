...
- Hypotheses:  
  ...
  - $H_a: \mu_1 \neq \mu_2$
  - 95% CI: $\mu_1 - \mu_2$
    - 0 not in the CI supports $H_a$  

### Proportion difference
- Parameters:  
  - $\pi_1$: probability in group 1  
  - $\pi_2$: probability in group 2  
- Hypotheses:  
  - $H_0: \pi_1 = \pi_2$  
  - $H_a: \pi_1 \neq \pi_2$
  - 95% CI: $\pi_1 - \pi_2$ 
...
- Small differences can still be important:
  - $\pi_1 - \pi_2 = 0.05 - 0.01 = 0.04$
  - The CI may suggest a very small difference.
  - It measures the absolute difference.
  - But this ignores relative scale:
    - 5% is 5 times larger than 1%!    
...
### Odds Example
- Suppose the odds of getting cancer = 1 to 2 → $\omega = \frac{1}{2} = 0.5$
...
- Odds/Probability Relationship:
  - Let $\omega_c$ = odds of cancer, $\pi_c$ = probability of cancer, $\1 - pi_c$: probability of no cancer
  - $ \omega_c = \frac{\pi_c}{1 - \pi_c} $

#### Odds Interpretation
- $0 < \omega < 1$: not in favor
  - Odds can't be a negative number
- $\omega = 1$: 50/50  
- $\omega > 1$: in favor  
