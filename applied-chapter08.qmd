...
- Where $\frac{\omega_1}{\omega_2}$ = odds ratio  
  - The odds ratio is always greater than 0.  
  - If $> 1$, the odds are higher in group 1   
  - If $< 1$, the odds are lower in group 1  
      
Note: Odds are not probabilities! Avoid words like "chance" or phrases like "x times more likely" when interpreting an odds ratio.  
...
- Put the larger proportion on the top of the ratio to keep the interpretation of the multiplier always greater than one.  
- When using software:  
...
4. **Run test and CI**:
   - Fisher’s exact test for small/moderate n  
   - Chi-squared test for large n  
   - Wald intervals for large n  
     - Use continuity correction if needed  
   - Fisher's bootstrap or Agresti-Coull, CIs for small n      
5. **Interpret results**:
