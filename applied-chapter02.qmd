...
**Key Idea:** Instead of minimizing just the residual sum of squares (RSS) to estimate coefficients, penalized regression minimizes the RSS plus a penalty term controlled by its own parameter, $\lambda$.
$$ RSS + \lambda \sum |\beta_j|^p $$
where $p=1$ for Lasso and $p=2$ for Ridge, and $\lambda$ is a penalty parameter.
...
- Determined using k-fold cross-validation.
  - Use an MSE plot to compare different $\lambda$s.
- Larger penalties lead to smaller, simpler models.
- The penalty is a function of both the penalty term and the coefficient values. A larger penalty term increases the total penalty, and the only way to minimize RSS + penalty is to shrink or eliminate some coefficients.
