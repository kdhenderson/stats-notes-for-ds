...
- **Remedy:** Consider adding quadratic or cubic terms to improve model fit.
...
- **High leverage, low influence:** Consistent with trend but far from most data. Does not change correlation, standard error, or parameter estimates significantly.
...
|-------------------------|--------------------|-------------|
| Means? linear, SD? constant | Model fits well | No action needed |
| Means? curved, SD? equal | Nonlinearity | Transform $X$ |
| Means? curved, SD? increasing | Nonlinearity + heteroscedasticity | Transform $Y$ |
| Skewed residuals | Non-normality | Can model mean, but confidence and prediction intervals will be compromised. Consider transformations |
| Means? linear, SD? increasing | Heteroscedasticity | Use weighted regression |
...
- $H_0$: Linear regression model has good fit. (No lack of fit.)
