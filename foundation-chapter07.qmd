...
- There are 5 groups, which gives $K = \binom{5}{2} = 10$ (5 choose 2) pairwise comparisons.
- Without correction, the risk of at least one false positive (rejecting $H_0$ when it is true) increases.
- Bonferroni correction provides a simple solution by adjusting the significance threshold.
...
To control the familywise error rate, adjust $\alpha$ using Bonferroni:

- Adjusted threshold: $\alpha' = \frac{\alpha}{K} = \frac{0.05}{10} = 0.005$
- Use Bonferroni-adjusted confidence intervals and *p*-values to control error rate across the family of comparisons.
...
- Each *p*-value is multiplied by $K$ before comparing to the original $\alpha$
...
- This protects against Type I error across the family of tests.
- Conservative, but appropriate when many unplanned comparisons are being made.
