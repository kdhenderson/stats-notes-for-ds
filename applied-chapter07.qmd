...
- Parametric models  
  - Always have been of the "predictive probability" type  
    - Logistic regression  
    - LDA/QDA  
    - Naive Bayes  
  - To convert the "predictive probability" model into a classification rule, typically use a thresholding technique  
    - Typically it is 0.5 (a 50/50 chance)  
    - The 0.5 can be changed to incorporate the cost of making an incorrect decision.  
- Nonparametric Models  
  - Originally just made decisions (not probabilities)  
    - Decision trees (classification trees)  
    - Random forest  
    - KNN  
  - Retroactively added predicted probabilities in software to help standardize functions and workflows  
- KNN Classification Formally  
  - Record the response values of the $k$ nearest neighbors to the new explanatory variable $x_{new}$. 
  - Predicted probabilities are computed as: $\hat{P}(Y = +class | x_{new}) = (# of times +class observed) / k$  
- Every model, once converted to a classification rule, has an associated classification boundary.  
- Classification boundaries are helpful in gaining intuition on:  
  - parametric vs. non-parametric classification models  
  - models with low/high complexity  
  
**Linear and Quadratic Discriminant Analysis (Discriminant Analysis**  
- Parametric approach  
  - Only uses numeric predictors (like KNN)  
  - Predictors are assumed to be MVN for each response class  
- Common Discriminant Analyses  
  - Linear discriminant analysis  
    - Linear (hyperplanes) decision boundaries  
  - Quadratic discriminant analysis  
    - Decision boundaries are conic sections  
    - Typically reduce to quadratic boundaries  
    - Little more flexibility than linear  

- Assumptions  
  - $X_+ = (X_1, X_2, ..., X_p)$ is a set of $p$ numeric predictors for the positive response class  
  - $X_- = (X_1, X_2, ..., X_p)$ is a set of the same $p$ numeric predictors for the negative response class  
  - LDA 
    - assumptions on predictors (not error terms as in MLR)  
    - $X_+ ~ MVN(\mu_+, \Sigma)$  
    - $X_- ~ MVN(\mu_-, \Sigma)$  
    - Predictor sets can have different mean vectors for each response, which is what we hope to see (to have separation between the classes) 
    - Variance-covariance matrix is the same  
      - The sigma symbol is the same in both the positive and negative class  
      - The variables in each group should have the same variance and the same correlation between them  
  - QDA  
    - Relaxes the assumptions (sigma can be different between classes)  
    - $X_+ ~ MVN(\mu_+, \Sigma_+)$  
    - $X_- ~ MVN(\mu_-, \Sigma_-)$   
    - Predictor sets can have different mean vectors for each response class  
    - Allows for the variance-covariance matrix to be different for each response class  

- Technical Insights  
  - LDA/QDA uses Bayes' theorem to perform its predicted probabilities similar to naive Bayes.  
  - Bayes' theorem says, for events A and B: $P(A|B) = \frac{P(B|A)P(A)}{P(B|A)P(A) + P(B|A^C)P(A^C)}$  
    - Where $A^C$ is A compliment. If A represents the 'yes' group, then $A^C$ represents the 'no' group.  
    - Example: if we let A = + class and B = X (set of predictor variables), Bayes' theorem provides a predicted probability formula:
$$
P(+|X) = \frac{P(X|+)P(+)}{P(X|+)P(+) + P(X|-)P(-)}
$$
      - Based on the assumptions, we know that the predictors follow a MVN and update Bayes' formula.     
$$
P(+|X) = \frac{MVN(\mu_+, \Sigma)P(+)}{MVN(\mu_+, \Sigma)P(+) + MVN(\mu_-, \Sigma)P(-)}
$$  
      - P(+) and P(-) are referred to a prior probabilities.  
        - Should reflect the frequency in which the two classes occur without any predictor information  
        - Handled automatically in software as long as your data is a random sample  
    - The formula is completely specified as long as we know the prior information, the value of the predictor X, the mean vector, and covariance matrix.  
    - All can be estimated from data.  
    - LDA's Boundary: if we use a majority rules (probability of 0.5) reduces to a linear boundary mathematically ??  
    - Outliers affect means and standard deviations; therefore they affect the classification boundary  
    - The prior probabilities can affect the classification boundary.  
    - Bayes' theorem can be extended to handle more than two classes.  
    
- Complexity of QDA  
  - Creates more parameters to estimate in the model  
    - The more predictors the more parameters, parameters increase at a greater rate (where LDA these increase at the same rate).  
    - Increases the chances of overfitting. QDA overfitting dampens with increasing sample size.  
    
- LDA/QDA Pros  
  - Parametric model that can predict when the training data observations are completely separated (logistic regression can't)  
  - Uses the fact that predictors are potentially correlated (OK as long as not perfectly correlated)  
  - Is the optimal method to use if the assumptions are met  
  
- LDA/QDA Cons  
  - Suffers from the curse of dimensionality; will not predict well if you include too many unhelpful predictors  
  - No built-in feature selection  
  - Cannot utilize important categorical predictors since assumptions are MVN  
  - Can be impacted by outliers  
