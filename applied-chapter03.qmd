...
- **Why It Works**
  - Each model's predictions have their own variance (Mean Squared Error, MSE).
  - Averaging predictions reduces variance — the variance of an ensemble is generally smaller than the variance of individual models. MSE tends to decrease in larger ensembles.
  - Helps smooth out overfitting by reducing variance.
  - However, the variance reduction is not as strong as in traditional averaging (e.g. averaging independent sample means, $X\bar$s) because all models are trained on the same dataset.
