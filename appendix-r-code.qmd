...
```{r}
# simple base R plots for checking assumptions
hist(dataset$numericalColumn)
qqnorm(dataset$numericalColumn)
boxplot(dataset$numericalColumn, horizontal = TRUE)


# make plots using ggplot2 and patchwork
library(patchwork)

# make a histogram for each treatment group
hist = dataset %>% 
  ...
  ggtitle("Histogram of ResponseVariable") +
  theme_bw()

# make a qqplot for each treatment group
qq = dataset %>%
  group_by(explanatory) %>% 
  ggplot(aes(sample = response)) +
  geom_qq() +
  facet_wrap(~explanatory) +
  ggtitle("QQ Plots of ResponseVariable") +
  theme_bw()

# make a boxplot for each treatment group
box = dataset %>% 
  group_by(explanatory) %>% 
  ggplot(aes(y = response, x = explanatory)) +
  geom_boxplot() +
  ggtitle("Boxplot of ResponseVariable") +
  theme_bw()

# arrange histogram above qqplot, then next to boxplot using patchwork
(hist / qq) | box


# simpler way for subset of data
par(mfrow=c(2,2))
hist(subset(dataset, explanatory==level1)$response, xlab='Response, Level 1', main='Level 1')
box()
hist(subset(dataset, explanatory==level2)$response, xlab='Response, Level 2', main='Level 2')
box()
qqnorm(subset(dataset, explanatory==level1)$response)
qqnorm(subset(dataset, explanatory==level2)$response)
