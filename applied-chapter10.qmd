...
- For multiple predictors $X = (X_1, X_2, X_p)$, the general logistic regression model takes the form:  
$$log \Big( \frac{p(X)}{(1-p(X))}\Big)= \beta_0 + \beta_1X_1 + \beta_2X_2 + ... + \beta_1X_p$$  
- Additive Interpretation  
  - When each predictor shows up only once in the model:  
    - $e^{\hat{\beta}}$ has an odds ratio interpretation  
    - Add the caveat, "holding the other variables fixed"  
- Complex Interpretation  
  - Logistic model has complexity  
    - Interaction terms  
    - Polynomial terms  
  - Odds ratios can still be used to explain  
    - Must carefully consider the regression formula (like in MLR)  
    - Common approach is to use effects plots (graphical assessment of logistic regression model)  
- Additive Classification Boundaries  
  - Classification boundaries are linear (numeric only)  
  - Similar to LDA but does not require predictors to be normally distributed  
- Complex Boundaries  
  - Interaction terms behave similarly to MLR  
    - Categ/categ: N/A  
    - Categ/cont: unique boundary for each categorical level  
    - Cont/cont or polynomial: creates nonlinear boundaries, can have multiple boundaries  
