...
- Results depend on choices made (distance, standardization, linkage).
- No perfect method for determining cluster size.
- Avoid interpreting a single result as absolute truth.
- Examine multiple clustering solutions for general themes.
...
- Clustering variables (i.e. clustering on the columns instead of rows by transposing the data matrix).
