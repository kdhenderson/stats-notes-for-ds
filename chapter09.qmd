...
- **Theoretical Model:**
  ...
  y = \beta_0 + \beta_1 x + \varepsilon
  $$
  - $y = \text{mean} \pm \text{residual}$, where the residual is the error between the mean and the observation.
  - Residuals ($\varepsilon$) are normally distributed with mean zero and constant variance ($\sigma^2$).
